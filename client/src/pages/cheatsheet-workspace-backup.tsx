...
export default function CheatSheetWorkspace() {
  ...
  const LAYOUT_CONFIG = {
    ...
    maxBoxHeight: 400,
    get gutter() {
      // Adaptive gutters based on density
      const gutterFactors = { compact: 8, balanced: 12, spacious: 16 };
      return gutterFactors[layoutDensity] || 12;
    },
    get contentWidth() { return this.pageWidth - (2 * this.margin); },
    get contentHeight() { return this.pageHeight - (2 * this.margin); },
    get columns() { 
      // Responsive columns with window size awareness
      const baseColumns = { compact: 4, balanced: 3, spacious: 2 };
      const windowWidth = typeof window !== 'undefined' ? window.innerWidth : 1200;
      const availableWidth = windowWidth - 256 - 448; // Subtract sidebars
      
      let columns = baseColumns[layoutDensity] || 3;
      
      // Adjust for smaller screens
      if (availableWidth < 800) columns = Math.min(columns, 2);
  ...
  // Working smart layout system
  const applySmartLayout = () => {
    if (boxes.length === 0) return;
    
    setDraggedBoxes(new Set());
    const layoutPositions = calculateMasonryLayout(boxes);
    ...
    toast({
      title: "Smart layout applied",
      description: `${boxes.length} boxes arranged with ${layoutDensity} density.`,
    });
  };
    
    if (allBoxes.length === 0) return [];
    
    // Calculate responsive viewport positioning
    const pageOffset = 20;
    const estimatedMiddlePanelWidth = typeof window !== 'undefined' ? window.innerWidth - 256 - 448 : 800;
    const centerOffsetX = Math.max(20, (estimatedMiddlePanelWidth - pageWidth) / 2);
    
    // Analyze and size all boxes with content awareness
    const boxesWithSizes = allBoxes.map(box => {
      // Skip layout calculation for manually dragged boxes
      if (draggedBoxes.has(box.id) && box.position) {
        return {
          ...box,
          optimalSize: {
    ...
    // Enhanced sorting with content type clustering
    boxesWithSizes.sort((a, b) => {
      // Skip sorting for manually positioned boxes
      if (a.optimalSize.contentType === 'manual') return 1;
      if (b.optimalSize.contentType === 'manual') return -1;
      
      // Priority-based sorting with area considerations
      ...
      // Secondary sort by content type for visual clustering
      const typeOrder = { 'matrix': 5, 'complex-math': 4, 'math': 3, 'image': 2, 'long-text': 2, 'standard': 1 };
      const typeA = typeOrder[a.optimalSize.contentType] || 1;
      const typeB = typeOrder[b.optimalSize.contentType] || 1;
      
      if (typeA !== typeB) {
        return typeB - typeA;
      }
      
      // Tertiary sort by area for optimal packing
      return b.optimalSize.area - a.optimalSize.area;
    });
    
    // Dynamic column width calculation with content awareness
    const baseColumnWidth = (contentWidth - (columns - 1) * gutter) / columns;
    const positions: Array<{ x: number; y: number; width: number; height: number }> = [];
    
    // Enhanced page-aware column tracking
    const pageColumnHeights: Record<number, number[]> = {};
    let currentPageIndex = 0;
    ...
    // Advanced masonry placement with intelligent overflow handling
    boxesWithSizes.forEach((box, index) => {
      ...
      while (!placed && attempts < maxAttempts) {
        ...
        // Intelligent column selection with load balancing
        let targetColumn = 0;
        let minHeight = columnHeights[0];
        
        // Find optimal column considering content fit and load balance
        for (let col = 0; col < columns; col++) {
          const projectedHeight = columnHeights[col] + height + gutter;
          
          if (columnHeights[col] < minHeight && projectedHeight <= contentHeight + gutter) {
            minHeight = columnHeights[col];
            targetColumn = col;
          }
        }
        
        // Advanced page boundary checking with look-ahead
        const wouldFitY = columnHeights[targetColumn] + height;
        const hasSpaceOnPage = wouldFitY <= contentHeight;
        const isFirstBoxOnPage = columnHeights.every(h => h === 0);
        
        if (hasSpaceOnPage || isFirstBoxOnPage) {
          // Calculate final positioning with responsive adjustments
          const pageYBase = pageOffset + margin + 40 + (pageIndex * (pageHeight + 40));
          
          // Dynamic width allocation with column spanning for wide content
          let finalWidth = Math.min(width, baseColumnWidth);
          let columnsSpanned = 1;
          
          // Allow wide content to span multiple columns if beneficial
          if (box.optimalSize.shape === 'wide' && width > baseColumnWidth * 1.3 && 
              targetColumn < columns - 1 && 
              Math.abs(columnHeights[targetColumn] - columnHeights[targetColumn + 1]) < gutter * 2) {
            const spanWidth = baseColumnWidth * 2 + gutter;
            if (width <= spanWidth) {
          ...
          const x = centerOffsetX + margin + targetColumn * (baseColumnWidth + gutter);
          const y = pageYBase + columnHeights[targetColumn];
          
          // Update column heights based on spanning
          for (let span = 0; span < columnsSpanned; span++) {
            const colIndex = targetColumn + span;
            if (colIndex < columns) {
              columnHeights[colIndex] += height + gutter;
            }
          }
          ...
          // Intelligent page overflow handling
          pageIndex++;
          if (pageIndex > currentPageIndex) {
            currentPageIndex = pageIndex;
          }
      ...
      // Fallback positioning if placement fails
      if (!placed) {
        const fallbackPageIndex = Math.floor(index / LAYOUT_CONFIG.targetBoxesPerPage);
        const fallbackX = centerOffsetX + margin + (index % columns) * (baseColumnWidth + gutter);
        const fallbackY = pageOffset + margin + 40 + (fallbackPageIndex * (pageHeight + 40)) + 
                         Math.floor((index % LAYOUT_CONFIG.targetBoxesPerPage) / columns) * (height + gutter);
        ...
        if (originalIndex !== -1) {
          positions[originalIndex] = { 
            x: fallbackX, 
            y: fallbackY, 
            width: Math.min(width, baseColumnWidth), 
            height 
          };
        }
      }
    ...
    return positions;
  }, [layoutDensity, boxesPerPage, draggedBoxes]);
